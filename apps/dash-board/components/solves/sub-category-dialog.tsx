...
type SubCategoryDialogBaseProps = {
  mainCategories: MainCategory[];
};

type SubCategoryDialogProps = SubCategoryDialogBaseProps &
  (
    | {
        mode: "add";
        action: SafeFunction<AddSubCategoryData, any>;
        selectedCategoryId?: number;
      }
    | {
        mode: "edit";
        action: SafeFunction<UpdateSubCategoryData, any>;
...
export function SubCategoryDialog(props: SubCategoryDialogProps) {
  ...
  const initialMainId =
    props.mode === "edit"
      ? props.category.mainId.toString()
      : props.mode === "add" && props.selectedCategoryId
      ? props.selectedCategoryId.toString()
      : "";

  const [name, setName] = useState(initialCategory?.name || "");
  const [mainId, setMainId] = useState<string>(initialMainId);
  ...
  useEffect(() => {
    if (props.mode === "edit") {
      ...
      setMainId(props.category.mainId.toString());
      setDescription(props.category.description || "");
      setAiPrompt(props.category.aiPrompt || "");
    }
  }, [props.mode === "edit" && props.category]);

  // add mode에서 selectedCategoryId가 변경되면 mainId 동기화
  useEffect(() => {
    if (props.mode === "add" && props.selectedCategoryId) {
      setMainId(props.selectedCategoryId.toString());
    }
  }, [props.mode === "add" && props.selectedCategoryId]);
  ...
  const resetForm = () => {
    if (props.mode === "add") {
      setName("");
      setMainId(
        props.selectedCategoryId ? props.selectedCategoryId.toString() : ""
      );
      setDescription("");
      setAiPrompt("");
    } else {
  ...
  const isValid = name.trim().length > 0 && mainId.length > 0;
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      ...
      <DialogContent className="sm:max-w-[500px]">
        <DialogHeader>
          <DialogTitle>
            {props.mode === "add" ? "중분류 추가" : "중분류 수정"}
          </DialogTitle>
          <DialogDescription>
            {props.mode === "add"
              ? "선택한 대분류에 새로운 중분류를 추가합니다."
              : "중분류 정보를 수정합니다."}
          </DialogDescription>
        </DialogHeader>
        <form onSubmit={handleSubmit} className="space-y-4">
          <div className="grid gap-2">
            <Label htmlFor="mainId">
              대분류 <span className="text-destructive">*</span>
            </Label>
            <Select
              value={mainId}
          ...
          <div className="grid gap-2">
            <Label htmlFor="name">
              중분류 이름 <span className="text-destructive">*</span>
            </Label>
            <Input
              id="name"
              placeholder="예: 대분류와 연관성 있는 중분류 이름(슬픈 영화, 코미디 영화..)"
              value={name}
