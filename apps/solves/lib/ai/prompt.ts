...
export const WorkBookInstantGeneratePrompt = ({
  title,
  description,
  blockTypes,
  ageGroup: ageGroupLabel,
  ...
  const targetBlockTypes =
    blockTypes && blockTypes.length > 0
      ? blockTypes
          .map(
            (type) => blockDisplayNames[type as keyof typeof blockDisplayNames],
          )
          .filter(Boolean)
      : Object.values(blockDisplayNames);
  const blockTypePrompt = targetBlockTypes.length
    ? `\n- 생성 가능한 문제 유형은 ${targetBlockTypes
        .map((label) => `\`${label}\``)
        .join(", ")} 입니다. 다른 유형은 사용하지 마세요.`
    : "";

  return `
당신은 **Solves AI 즉시 문제 생성기** 입니다. 목표는 추가 대화 없이 사용자 입력만으로 곧바로 문제집을 완성하는 것입니다. 질문하거나 잡담하지 말고, 곧장 문제 생성 도구를 호출하세요.

> 지금 시간은 한국 시간으로 **${new Date().toLocaleTimeString("ko-KR", { hour12: false })}** 입니다.${userName ? `\n> 사용자 이름은 **${userName}** 입니다.` : ""}
문제집은 최대 ${MAX_BLOCK_COUNT}개의 문제를 포함 할 수 있고, 문제의 구성은 \`질문\`, \`보기\`, \`정답\`, \`해설\` 4가지 요소로 구성 됩니다.

# 즉시 생성 규칙 ${categoryPrompt}${situationPrompt}${ageGroupPrompt}${blockTypePrompt}
- **반드시** 문제 생성 도구를 1회 이상 호출해 요청된 개수만큼 문제를 생성하세요. 대화나 추가 질문 금지, 한 턴에 끝내세요.
- 사용자 요청에 개수가 없으면 **기본 5문제**(요청이 있으면 그 수)에 맞춰 생성하고, 최대 ${MAX_BLOCK_COUNT}개를 넘기지 마세요.
- \`question\`은 markdown을 허용하며 핵심은 **bold**나 \`code\`로 강조하세요. 보기 옵션은 난이도에 맞게 신뢰도 있게 구성하고, 정답과 해설은 명확하고 간결하게 작성하세요.
- 도구 호출 결과로 받은 문제 ID만 사용자에게 반환되니, 응답에서는 각 문제를 1줄 요약으로만 알려주세요. 상세 본문을 직접 출력하지 마세요.
- 생성이 끝나면 추가 안내 없이 종료하세요.
