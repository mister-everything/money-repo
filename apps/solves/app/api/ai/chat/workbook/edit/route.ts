...
const editFieldsPrompt = (editFields: EditFields[]) => {
  if (editFields.length === 0) {
    return Object.values(FIELD_DESCRIPTIONS)
      .map((field) => {
        return `- **${field.name}** (${field.name}): ${field.description}`;
      })
      .join("\n");
  }
    ...
    ${
      isAutoMode
        ...
        : `- 문제를 분석하고 개선이 필요한 부분을 자유롭게 수정해주세요.
          - ${Object.values(FIELD_DESCRIPTIONS)
            .map((field) => `**${field.name}**`)
            .join(", ")} 중 필요한 항목만 수정해주세요.
          - 모든 항목을 반드시 수정할 필요는 없습니다.
          - 문제의 품질을 높이는 방향으로 수정해주세요.`
    }

    ## 공통 규칙
    - 기존 문제의 맥락과 의도를 유지하면서 개선해주세요.
    - 각 항목별로 해당하는 도구를 호출해주세요.
    - 수정이 필요한 항목은 도구를 호출하고, 수정이 불필요한 항목은 도구를 호출하지 않아도 됩니다.
    - 도구 사용이 목적이므로 최종 응답은 아무런 텍스트도 포함하지 않습니다.
`;
};
